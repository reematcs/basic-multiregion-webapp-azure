...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        console.error('Could not fetch metrics:', metricsError);
        setMetrics([]);
      }
  ...
  const handleFailover = async () => {
    try {
      ...
      // Determine target region based on current role
      const targetRegion = healthStatus?.role === 'primary' ? 'centralus' : 'westus';
      ...
      const pollStatus = async () => {
        ...
        const newHealthRes = await fetch('/api/health/status');
        const newHealth = await newHealthRes.json();

        if (newHealth.role !== healthStatus.role) {
          setFailoverInProgress(false);
        } else {
          pollStatus(); // Continue polling
        }
      };
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {/* Health Status and Region Status */}
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Button
              ...
              className="mt-4 w-full bg-blue-500 hover:bg-blue-600 text-white transition-colors disabled:bg-gray-400"
            >
              <ArrowRightLeft className={`w-4 h-4 mr-2 ${failoverInProgress ? 'animate-spin' : ''}`} />
              {failoverInProgress ? 'Failover in progress...' : 'Trigger Failover'}
            </Button>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={metrics}>
                ...
                <XAxis dataKey="timestamp" className="text-gray-600" />
                ...
                <Line type="monotone" dataKey="requestCount" stroke="#16a34a" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
